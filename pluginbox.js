...
(function(window, document) {
    ...
    class PluginSystem {
        ...
        installPlugin(code) {
            ...
            // 检查是否已安装
            if (this.isPluginInstalled(manifest.packname)) {
                this.showError('plugin-code-error', `已安装同名插件: ${manifest.packname}`);
                return false;
            }
        ...
        runPlugin(code) {
            ...
            pluginFunction(this.getPluginAPI());
        }
        ...
        getPluginAPI() {
            return {
                ...
                // 通知主应用的方法
                notify: (message, type = 'info') => this.notify(message, type),
                ...
                // 窗口系统API
                createWindow: (windowName, x, y) => this.createWinbox(windowName, x, y),
                toggleWindow: (winboxId, x, y) => this.toggleWinbox(winboxId, x, y),
                closeWindow: (winboxId) => this.closeWinbox(winboxId),
                
                // 拼图按钮拖动控制API
                toggleButtonDrag: () => this.toggleButtonDragMode(),
                unlockButtonDrag: () => {
                    if (!this.isDraggingButton) {
                        this.toggleButtonDragMode();
                    }
                ...
                resetButtonPosition: () => this.resetButtonPosition()
            };
        }
